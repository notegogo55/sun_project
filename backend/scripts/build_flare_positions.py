...
ตัวอย่างการใช้งาน::
    ...
    python backend/scripts/build_flare_positions.py --csv D:/position_flare/backend/source_data/out/flares_all_cycles.csv
    python backend/scripts/build_flare_positions.py --tolerance-min 5
...
def main() -> int:
    ...
    if not catalog_path.exists():
        logger.error("ไม่พบแคตตาล็อกของโมเดลที่ %s — รัน backend/scripts/download_metadata.py ก่อน", catalog_path)
        return 1
