...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--no-split",
        action="store_true",
        help="ข้ามการแบ่ง train/val/test (ใช้ตอนทดสอบด้วยข้อมูลช่วงสั้น หรือเตรียม pool สำหรับ --cv)",
    )
    p.add_argument(
        "--out-dir",
        type=Path,
        default=None,
        help="override ที่เขียนผลลัพธ์ (ปริยาย data/processed/sequences) — ใช้กับ --no-split "
        "เพื่อไม่ให้ทับ pool ของ production เช่น --out-dir data/processed/sequences_cv",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    out_dir = args.out_dir or cfg.paths.processed / "sequences"
    ...
    logger.info("ขั้นถัดไป:  python backend/scripts/forecast/train.py --model lstm")
    logger.info("=" * 70)
    return 0
