...
ตัวอย่างการใช้งาน::
    ...
    python backend/scripts/data/download_metadata.py --start 2014-01-01 --end 2014-03-01
    ...
    python backend/scripts/data/download_metadata.py --start 2018-01-01 --end 2021-12-31 --append
...
def main() -> int:
    ...
    if args.skip_flares and flares_path.exists():
        ...
        if flares.empty:
            logger.error("ไม่ได้รับ flare event เลย — รัน backend/scripts/checks/check_connectivity.py เพื่อวินิจฉัย")
            return 1
    ...
    logger.info("ขั้นถัดไป:  python backend/scripts/data/build_sequences.py")
    logger.info("=" * 70)
    return 0
