"""ดาวน์โหลดภาพ AIA สามชั้นบรรยากาศ แล้ววางลงกริดเดียวกับเฟรม HMI ที่มีอยู่
    ...
    python backend/scripts/data/download_aia.py --wcs-only

    # 1) ทดสอบด้วยไม่กี่เฟรมก่อนเสมอ
    python backend/scripts/data/download_aia.py --start 2014-10-20 --end 2014-10-28 --limit 5
    ...
    python backend/scripts/data/download_aia.py
...
def main() -> int:
    ...
    if not stems:
        logger.error(
            "ไม่พบเฟรมในช่วงเวลานี้ที่ %s — รัน backend/scripts/data/download_images.py ก่อน",
            frames_dir / "images",
        )
        return 1
    ...
    if args.wcs_only:
        ...
        logger.info("ขั้นถัดไป: python backend/scripts/data/download_aia.py --limit 5")
        logger.info("=" * 70)
        return 0
    ...
    logger.info("  1) ตรวจการจัดตำแหน่งด้วยตา:  python backend/scripts/checks/plot_aia_alignment.py")
