"""เรนเดอร์ผล segmentation ของ U-Net เป็นวิดีโอ (mp4) สำหรับช่วงเวลาที่กำหนด
    ...
    python backend/scripts/segmentation/render_segmentation_video.py --start 2024-05-01 --end 2024-06-01
    python backend/scripts/segmentation/render_segmentation_video.py --layer 171
    python backend/scripts/segmentation/render_segmentation_video.py --layer grid
    python backend/scripts/segmentation/render_segmentation_video.py --layer stack
    python backend/scripts/segmentation/render_segmentation_video.py --layer row --harp 11149  # Gannon storm (NOAA 13664)
...
def main() -> int:
    ...
    if not seg.available:
        logger.error(
            "ยังไม่มีโมเดล U-Net ที่ใช้งานได้ (%s) — รัน backend/scripts/segmentation/train.py ก่อน",
            seg.checkpoint_path,
        )
        return 1
    ...
    if needs_aia and not aia_store.available:
        logger.error(
            "ยังไม่มีภาพ AIA ที่ %s — รัน backend/scripts/data/download_aia.py ก่อน หรือใช้ --layer mag",
            aia_store.root,
        )
        return 1
    ...
    if args.harp is not None:
        ...
        if not frame_wcs.available:
            logger.error(
                "ไม่มี WCS จริงของเฟรมที่ %s — รัน backend/scripts/data/download_aia.py --wcs-only ก่อน",
                frame_wcs.path,
            )
            return 1
