...
from sunseg.training.losses import build_segmentation_loss  # noqa: E402
...
def main() -> int:
    ...
    if not image_dir.exists() or not any(image_dir.glob("*.npy")):
        logger.error(
            "ไม่พบเฟรมภาพใน %s\n"
            "รัน `python backend/scripts/data/download_images.py` แล้วตามด้วย `python backend/scripts/build_masks.py`",
            image_dir,
        )
        return 1
    ...
    if test_metrics["dice"] >= 0.70:
        ...
        logger.warning(
            ...
            "หรือตรวจว่า mask วางตรงตำแหน่งด้วย backend/scripts/segmentation/plot_masks.py",
            test_metrics["dice"],
        )
    logger.info("=" * 70)
    return 0
