...
ช่วงเต็ม (ticket 05 ของ ``.scratch/lstm-feature-ablation/``)
    ...
    # ทดสอบก่อนเสมอ
    python backend/scripts/study/extract_intensity.py --limit 20

    # รันเต็ม — หยุดกลางคันแล้วรันใหม่ได้ เฟรมที่เสร็จแล้วถูกข้าม
    python backend/scripts/study/extract_intensity.py

    # รวมผลที่สกัดไว้แล้วเป็นไฟล์/รายงานใหม่ โดยไม่ประมวลผลเฟรมเพิ่ม
    python backend/scripts/study/extract_intensity.py --report-only
...
- ``intensity_full_range.parquet`` — schema เดียวกับ ``gate_check/intensity_may2024.parquet``
  (``study/build_dataset.py`` หยิบทุกไฟล์ ``intensity*.parquet`` ใต้ ``intensity_study/`` เอง
...
- ``parts/`` — ผลรายรอบที่ทำให้รันต่อได้ ตั้งชื่อ ``rows_*``/``ledger_*`` **ไม่ขึ้นต้นด้วย
  ``intensity``** โดยตั้งใจ ไม่ให้ ``study/build_dataset.py`` หยิบซ้ำ
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out-subdir", type=Path, default=OUT_SUBDIR,
        help=f"ที่เก็บผลใต้ artifacts/ (ปริยาย {OUT_SUBDIR}) — ถ้าไม่อยู่ใต้ intensity_study/ "
        "study/build_dataset.py ปริยายจะไม่หยิบไปปนกับตารางเดิม",
    )
    p.add_argument(
        "--unet", type=Path,
        help="checkpoint ของ U-Net (ปริยาย artifacts/models/unet.pt) — ตาราง intensity เดิมสกัดด้วย "
        "models/unet_prev_412samples.pt ก่อนที่ unet.pt จะถูกแทนด้วยรอบเทรนเฟรมเต็ม",
    )
    p.add_argument("--flares-path", type=Path, help="flare catalog สำหรับรายงาน (ปริยาย data/interim/flares.parquet)")
    return p.parse_args()
...
def main() -> int:
    ...
    out_dir = cfg.paths.artifacts / args.out_subdir
    ...
    if not args.report_only:
        ...
        if pending:
            ...
            unet_path = args.unet or cfg.paths.artifacts / "models" / "unet.pt"
            segmentation = SegmentationService(unet_path, frames_dir=frames_dir, device=device)
            if not segmentation.available:
                logger.error("ยังไม่มีโมเดล segmentation — รัน backend/scripts/segmentation/train.py ก่อน")
                return 1
            logger.info("U-Net %s รันบน %s", unet_path, device)
    ...
    flares = pd.read_parquet(args.flares_path or cfg.paths.interim / "flares.parquet")
