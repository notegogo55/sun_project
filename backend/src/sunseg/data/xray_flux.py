...
#: ดาวเทียมที่อ่าน **เรียงตาม priority จริง ๆ** — ``_day_path`` หยิบดวงแรกที่มีไฟล์ของ
#: วันนั้น ไม่ใช่ดวงที่ดีที่สุด เพราะฉะนั้นลำดับนี้คือการตัดสินใจ ไม่ใช่การจัดเรียงเฉย ๆ
#:
#: เดิมมีแค่ ("g15", "g16") ซึ่งพอสำหรับ 2011-2017 + พ.ค. 2024 และตอนนั้นช่วงเวลาของ
#: สองดวงแทบไม่ทับกันจึงไม่ต้องสนลำดับ  พอขยายไป 2022-2025 ดาวเทียมทับกันจริงหลายดวง
#: ลำดับจึงถูกเลือกตามหลัก "ใช้ดวงที่สายการคาลิเบรตสั้นที่สุดก่อน":
#:
#:   g15  ตัวอ้างอิง สเกล 1.0 พอดี ไม่ผ่านตัวคูณใด ๆ
#:   g16  วัดตรงกับ g15 บนช่วงทับซ้อนปี 2017 (สายยาว 1 ขั้น)
#:   g19  GOES-East ที่มารับช่วงต่อจาก g16 — ใช้เมื่อ g16 ไม่มีไฟล์แล้ว
#:   g18  GOES-West รุ่นปัจจุบัน — สำรองของ g19
#:   g17  GOES-West รุ่นก่อน — ท้ายสุด มีปัญหาระบบหล่อเย็นที่ทราบกันดี
#:
#: **ห้ามสลับลำดับโดยไม่วัดใหม่** การเปลี่ยนลำดับเปลี่ยนว่าวันไหนใช้ดวงไหน ซึ่งเปลี่ยน
#: ตัวคูณสเกลที่ถูกใช้ และเปลี่ยนค่า feature ที่โมเดลเห็น
SOURCE_SATELLITES: tuple[str, ...] = ("g15", "g16", "g19", "g18", "g17")
...
#:
#: g17/g18/g19 วัดเทียบ g16 บนช่วงทับซ้อน แล้วประกอบเข้ากับสเกลของ g16 เอง
#: (scale_target = scale_g16 / ratio) เพราะไม่มีช่วงไหนที่ g15 ทับกับสามดวงนี้เลย
SATELLITE_SCALE: dict[str, float] = {
    "g15": 1.0,
    "g16": 1.0 / 1.0805,
    # g19 วัดเทียบ g16 ไม่ใช่ g15 เพราะ **ไม่มีวันไหนเลยที่ g15 กับ g19 ทับกัน**
    # (g15 หยุด 2020-03-04 · g19 เริ่มมีข้อมูลปี 2024) จึงต้องประกอบสองขั้น:
    #
    #   g19/g15 = (g19/g16) x (g16/g15) = 0.9989 x 1.0805 = 1.07931
    #
    # เขียนเป็นผลคูณไว้ตรงนี้แทนที่จะยุบเป็น 0.926517 เพื่อให้เห็นที่มาทั้งสองขั้น —
    # ถ้าวันหนึ่งวัด g16/g15 ใหม่ได้ค่าต่างไป ตัวเลขของ g19 จะขยับตามเองโดยไม่ต้องจำว่า
    # มันถูกประกอบมาจากอะไร
    #
    # วัดบน 2024-01-01..2025-12-31: 228,421 จุดที่มีข้อมูลดีทั้งสองดวง (มากกว่าตอนวัด
    # g15/g16 ซึ่งใช้ 29,620 จุด ราว 8 เท่า) · median ทั้งช่วง 0.9989 · p10/p90 =
    # 0.9936/1.0082 · เฉพาะ M-class+ (n=8,589) ได้ 1.0030 · ที่จุดพีค 2024-10-03 ได้
    # 1.0128 — สามวิธีสอดคล้องกันในช่วง 0.999-1.013
    #
    # g19 กับ g16 ต่างกันแค่ 0.11% (คนละเรื่องกับ g15/g16 ที่ห่างกัน 8%) เพราะทั้งคู่
    # เป็น GOES-R series เครื่องมือและวิธีคาลิเบรตเดียวกัน ส่วน g15 เป็นคนละรุ่น
    #
    # ถ้าไม่ใส่ค่านี้ g19 จะถือสเกล 1.0 ซึ่งผิดไป 7.9% (= 0.033 หน่วยใน
    # xray_log10_median) ตลอด 269 วันของปี 2025 ที่ g19 เป็นดวงที่ถูกเลือก
    # ดูรายงานเต็มที่ artifacts/other_results/checks/xray_cross_calibration/report.md
    "g19": 1.0 / (0.9989 * 1.0805),
}
